...
**`code` — source evidence, and every block must be ONE COMPLETE THING (user rule 2026-07-27).** Each block quotes a single complete unit that backs up a `contents` claim: the whole rule text, the whole timer config, the whole table definition — **never a fragment cut mid-sentence or mid-structure**. Required per block: `title` (what this quote is, e.g. "The four rules, complete original text"), `src` (the file it comes from), `lines` (per-line plain translation), `n` (the one-sentence takeaway). 3–15 lines; if the complete thing is longer than 15 lines, quote its most decision-relevant part and say so in the title ("rules 1–2 of 4, from X"). The UI shows blocks collapsed under "Source evidence / 原文对照" — evidence is verification for the curious, never the primary explanation.
...
**`glossary` — the select-to-explain dictionary (required in v2).** Collect EVERY technical term your content uses (API, cron, Markdown, cache, webhook, 流水线, 工作记忆…) and write a top-level `glossary`: `{ "<term>": { "short": …, "detail": …, "example": … } }`. `short` (required) = 1–2 sentences a non-technical person gets on first read; `detail` (optional) = one step deeper; `example` (optional) = an everyday comparison that survives a follow-up question. Add BOTH the English and Chinese surface forms as keys when the languages differ (`"pipeline"` and `"流水线"` both point to entries). A term whose meaning is specific to one node goes in that node's `glossary` instead (node entries override global ones). Rule of thumb: if a reader might select it and tap "Explain", it should be here.
